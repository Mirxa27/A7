...
export const getIntelRecords = async () => {
    try {
        const response = await fetch('/api/intel');
        if (!response.ok)
            throw new Error('Failed to fetch intel records');
...
export const saveIntelRecord = async (record) => {
    try {
        const response = await fetch('/api/intel', {
            method: 'POST',
        ...
        if (!response.ok)
            throw new Error('Failed to save intel record');
...
export const saveMission = async (plan) => {
    const missionRecord = {
...
export const addSystemLog = (source, message, status = 'INFO') => {
