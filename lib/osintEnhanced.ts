...
export async function searchUsernameEnhanced(username: string) {
  ...
  const summary = {
    ...
    byType: processed.reduce((acc: any, curr) => {
      if (curr.exists) {
...
export async function checkEmailBreaches(email: string, apiKey?: string) {
  if (!apiKey) {
    return {
      email,
      checked: false,
