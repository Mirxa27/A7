...
export class ExportService {
  ...
  async generatePDFDossier(data: DossierData, options: ExportOptions = { format: 'pdf' }): Promise<string> {
  ...
  async generateJSON(data: any, options: ExportOptions = { format: 'json' }): Promise<string> {
  ...
  async generateCSV(data: any[], headers: string[], options: ExportOptions = { format: 'csv' }): Promise<string> {
  ...
  async generatePDFMission(data: MissionData, options: ExportOptions = { format: 'pdf' }): Promise<string> {
  ...
  getExportInfo(filepath: string): { filename: string; size: number; created: Date } | null {
  ...
  cleanupOldExports(maxAgeHours: number = 24): number {
  ...
  // List all exports
  listExports(): Array<{ filename: string; size: number; created: Date; path: string }> {
